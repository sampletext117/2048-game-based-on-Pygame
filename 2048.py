...
def start_screen(loaded_game=False):
    ...
    label_rules3 = rules_font.render(
        "F2 - pause music, F3 - unpause", 1, (0, 0, 0))
    ...
    for i in range(0, field_size):
        for j in range(0, field_size):
            pygame.draw.rect(surface,
                             colour_dict[matrix[i][j]],
                             (i * (width / field_size),
                              j * (width / field_size) + 100,
                                 (width / field_size) - 5,
                                 (width / field_size) - 5))
            if matrix[i][j] == 0:
                label = common_font.render('', 1, (255, 255, 255))
            else:
                label = common_font.render(
                    str(matrix[i][j]), 1, (255, 255, 255))
            label2 = score_font.render(
                "Score:" + str(total_points), 1, (0, 0, 0))

            if matrix[i][j] < 9:
                surface.blit(label, ((i * (width / field_size) +
                                      ((width / field_size) / 2.5)),
...
def set_game_over_labels():
    ...
    label2 = score_font.render(
        "Your score is " + str(total_points), 1, (0, 0, 0))
...
def save_game():
    name_with_field_size = "savedata" + \
                           str(field_size) + 'X' + str(field_size)
    f = open(name_with_field_size, "w")

    line1 = " ".join([str(matrix[make_round(_ /
                                            field_size)][_ % field_size])
...
def pressed_arrow_or_wasd(k):
    return (k == pygame.K_UP or k == pygame.K_DOWN or
            k == pygame.K_LEFT or k == pygame.K_RIGHT
            or k == pygame.K_w or k == pygame.K_s or
            k == pygame.K_a or k == pygame.K_d)
